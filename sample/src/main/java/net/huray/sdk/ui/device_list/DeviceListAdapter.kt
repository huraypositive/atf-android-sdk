...
class DeviceListAdapter(private val clickListener: DeviceItemClickListener) :
    ...
    private fun initDeviceItems() {
        ...
        deviceStates.add(
            DeviceStatus(
                AtfleeDeviceType.T8,
                PrefUtils.getDeviceAddress(AtfleeDeviceType.T8) != null
            )
        )
        deviceStates.add(
            DeviceStatus(
                AtfleeDeviceType.T9,
                PrefUtils.getDeviceAddress(AtfleeDeviceType.T9) != null
            )
        )
        deviceStates.add(
            DeviceStatus(
                AtfleeDeviceType.I_GRIP_X,
                PrefUtils.getDeviceAddress(AtfleeDeviceType.I_GRIP_X) != null
            )
        )
    }
}